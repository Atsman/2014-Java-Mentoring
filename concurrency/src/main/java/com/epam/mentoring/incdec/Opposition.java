package com.epam.mentoring.incdec;
...
public class Opposition {
    ...
    public class Counter {
        ...
        public synchronized void decrement() {
            while(count == 0) {
                try {
                    wait();
                    log.info("Count is zero. Waiting...");
    ...
    private void start() {
        ...
        t1 = new Thread(new Wrestler(counter, true), "Increment");
        t2 = new Thread(new Wrestler(counter, false), "Decrement");
        t2.start();
        t1.start();
        try {
            while (true) {
